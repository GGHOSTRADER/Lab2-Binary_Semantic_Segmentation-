...
def dice_loss_from_logits(
    ...
    Args:
        ...
        foreground_class:
            Foreground class index in the channel dimension.
        eps:
            Numerical stability constant.

    Returns:
        Scalar tensor: 1 - mean soft Dice
    """
    if logits.ndim != 4:
        raise ValueError(
...
# -----------------------------
# Validation using combined loss
# -----------------------------
@torch.no_grad()
def validate_one_epoch_combined(
    model: nn.Module,
...
print(f"Mode:              max (tracking val_dice)")
...
for epoch in range(NUM_EPOCHS):
    ...
    val_loss, val_dice = validate_one_epoch_combined(
        model=model,
